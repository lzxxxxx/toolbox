      ...
      //如果达到上限 清空所有操作记录（下次进来还有上次记录，因为下面要存储，不用去掉提示）
        INDEXDB.getItemMount(function(mount){
          ...
          for(var i = 0 ; i < data.length;i++){
            var store = store = myDB.db.transaction(myDB.storename,'readwrite').objectStore(myDB.storename),request;
