  ...
  class Toolbar {
    ...
    handleSave (){
      ...
      this.dom_obj.save_btn.on('click',function(evt){
        ...
        for(let item in Coordlist['inputs']){//清空全部输入框重新加
    ...
    subPixelText (ctx,text,x,y,fontHeight){
      var  _this = this;
 ...
 */
  var Common_func = {
    _setDownURL: function(val){
      //设置下载按钮的url
      window.Dom_obj.download_btn.attr('href',val);
    },
    _handleDrag: function (ele){
      //超出框的时候会被删除
    ...
    _handleInputevent: function(inputs){
